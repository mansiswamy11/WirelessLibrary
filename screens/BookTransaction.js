...
export default class TransactionScreen extends React.Component {
    ...
    checkStudentEligibilityForBookIssue = async () => {
        const studentRef = await db.collection("students").where(
            "studentId", "==", this.state.scannedStudentId).get();
    ...
    checkStudentEligibilityForReturn = async () => {
        const transactionRef = await db.collections("tranactions").where("bookId", "==", this.state.scannedBookId).limit(1).get();
        var isStudentEligible = "";
    ...
    handleTransaction = async () => {
        ...
        if (!transactionType) {
            ...
            if (isStudentEligible) {
                this.initiateBookReturn(); Alert.alert("Book returned to the library!");
            }
        }
    };
